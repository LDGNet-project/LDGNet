...
def compute_clf(pl_module, ret):
    ...
    p = getattr(pl_module, f"{phase}_clf_p")(ret["logits"], ret["label_ids"])
    r = getattr(pl_module, f"{phase}_clf_r")(ret["logits"], ret["label_ids"])
    pl_module.log(f"clf/{phase}/loss", loss)
    pl_module.log(f"clf/{phase}/accuracy", acc)
    pl_module.log(f"clf/{phase}/f1", f1)
    pl_module.log(f"clf/{phase}/p", p)
    pl_module.log(f"clf/{phase}/r", r)
