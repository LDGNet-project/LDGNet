...
from ..gadgets.my_metrics import Accuracy, VQAScore, Scalar, F1, P, R
...
def set_metrics(pl_module):
    for split in ["train", "val", "test"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            setattr(pl_module, f"{split}_{k}_p", P())
            setattr(pl_module, f"{split}_{k}_r", R())
...
def epoch_wrapup(pl_module):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if phase == "train":
            ...
            pl_module.log(
                f"{loss_name}/val/P",
                getattr(pl_module, f"val_{loss_name}_p").compute(),
            )
            getattr(pl_module, f"val_{loss_name}_p").reset()

            pl_module.log(
                f"{loss_name}/val/R",
                getattr(pl_module, f"val_{loss_name}_r").compute(),
            )
            getattr(pl_module, f"val_{loss_name}_r").reset()
