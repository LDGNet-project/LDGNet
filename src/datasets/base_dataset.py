...
class JsonDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if "image" not in self.data[0].keys():
            self.img_key = "images"
    ...
    def get_image(self, idx):
        ...
        return {
            ...
            # "img_path": f"{self.data_dir}/twitter/images/{str(self.data[idx][self.img_key])}",
    ...
    def collate(self, batch, mlm_collator):
        ...
        #z = list(0 if i[0] == 'n' else 1 for i in dict_batch["label"])
        # print("_-----------------------------")
        # print(dict_batch["label"])
        batch_labels = torch.LongTensor(dict_batch["label"])
        #batch_labels = torch.LongTensor(z)
